...
<div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto">
        <div class="container mx-auto px-4 py-8">
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold bg-gradient-to-r from-[#667eea] to-[#764ba2] bg-clip-text text-transparent mb-2">Sales Report</h1>
                <p class="text-gray-600">View and manage completed sales transactions</p>
            </div>

            <!-- Filter Card -->
            <div class="bg-white border border-gray-200 rounded-xl shadow-md p-6 mb-6">
                <form method="GET" action="{{ route('special-sales-report.index') }}" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                name="start_date"
                                value="{{ $startDate }}"
                                class="w-full h-11 px-4 bg-white border border-gray-300 rounded-lg text-gray-900 focus:ring-2 focus:ring-purple-500 focus:border-purple-500">
                        </div>
                        ...
                        <div class="flex items-end space-x-2 h-full">
                            <button
                                type="submit"
                                class="flex-1 h-11 bg-gradient-to-r from-[#667eea] to-[#764ba2] hover:shadow-lg hover:shadow-purple-500/50 text-white font-semibold px-4 rounded-lg transition duration-200 flex items-center justify-center">
                            ...
                            <a
                                ...
                                class="h-11 min-w-[110px] bg-gradient-to-r from-[#28a745] to-[#20c997] hover:shadow-lg hover:shadow-green-500/50 text-white font-semibold px-4 rounded-lg transition duration-200 flex items-center justify-center">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <table class="w-full">
                        <thead class="bg-gradient-to-r from-[#667eea] to-[#764ba2]">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Payment</th>
                                <th class="px-6 py-4 text-right text-xs font-semibold text-white uppercase tracking-wider">Cash</th>
                                <th class="px-6 py-4 text-right text-xs font-semibold text-white uppercase tracking-wider">Card</th>
                                <th class="px-6 py-4 text-right text-xs font-semibold text-white uppercase tracking-wider">Credit</th>
                                <th class="px-6 py-4 text-left text-xs font-semibold text-white uppercase tracking-wider">Date/Time</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            $serialNumber = ($orders->firstItem() ?? 1) + $loop->index;
                            @endphp
                            <tr class="{{ !empty($order->customer_vat_number) ? 'bg-yellow-50 hover:bg-yellow-100' : 'hover:bg-purple-50' }} transition-colors" data-order-id="{{ $order->id }}">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-purple-600 font-mono font-semibold">{{ $serialNumber }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @elseif($order->order_type == 'uber_eats') bg-yellow-600/20 text-yellow-400
                                    @elseif($order->order_type == 'pickme') bg-orange-600/20 text-orange-400
                                    @endif">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    @elseif($isMixed)
                                    <span class="inline-block px-3 py-1 text-xs font-bold rounded-md bg-purple-100 text-purple-700">Cash and Card</span>
                                    @else
                            ...
                            <tr>
                                <td colspan="9" class="px-6 py-12 text-center">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p class="text-gray-600 text-lg">No sales records found for the selected filters</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
            ...
            <div class="flex items-center justify-center min-h-screen px-4">
                ...
                <div class="relative bg-white rounded-xl shadow-2xl border border-gray-200 max-w-4xl w-full">
                    ...
                    <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-purple-50 to-purple-100">
                        <h3 class="text-xl font-bold text-purple-700">Sale Details</h3>
                    ...
                    <div class="px-6 py-4 max-h-[70vh] overflow-y-auto">
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-6">
                            <div>
                                ...
                                <p class="text-lg font-semibold text-gray-800" id="modal-order-number">-</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-600">Payment Number</p>
                                <p class="text-lg font-semibold text-gray-800" id="modal-payment-number">-</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-600">Waiter</p>
                                <p class="text-lg font-semibold text-gray-800" id="modal-waiter">-</p>
                            </div>

                            <div>
                                <p class="text-sm text-gray-600">Order Type</p>
                                <p class="text-lg font-semibold text-gray-800" id="modal-order-type">-</p>
                            </div>
                            <div>
                                <p class="text-sm text-gray-600">Date & Time</p>
                                <p class="text-lg font-semibold text-gray-800" id="modal-date">-</p>
                            </div>
                        </div>
                        ...
                        <div class="bg-purple-50 rounded-lg p-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex justify-between border-t border-purple-200 pt-2 mt-2">
                                    ...
                                    <span class="text-purple-600 font-bold text-lg" id="modal-total">-</span>
                                </div>
                                <div class="flex justify-between border-t border-purple-200 pt-2 mt-2">
        ...
        <div class="modal fade hidden fixed inset-0 z-50 overflow-y-auto" id="deleteConfirmModal">
            <div class="flex items-center justify-center min-h-screen px-4">
                <div class="modal-backdrop fixed inset-0"></div>
                <div class="relative bg-white rounded-xl shadow-2xl border border-gray-200 max-w-md w-full">
                    <!-- Modal Header -->
                    ...
                    <div class="px-6 py-4">
                        <div class="flex items-center mb-4">
                            ...
                            <div>
                                <h4 class="text-lg font-semibold text-gray-800 mb-1">Delete Order</h4>
                                <p class="text-gray-600 text-sm">Are you sure you want to delete order <span class="text-gray-800 font-semibold" id="delete-order-number">-</span>?</p>
                            </div>
                        </div>
                        <p class="text-gray-600 text-sm">This action will mark the order as deleted. This cannot be undone.</p>
                    </div>

                    <!-- Modal Footer -->
                    <div class="flex items-center justify-end space-x-3 px-6 py-4 border-t border-gray-200 bg-gray-50">
                        <button class="bg-white hover:bg-gray-100 text-gray-700 font-semibold py-2 px-6 rounded-lg transition close-delete-modal border border-gray-300">
                            Cancel
                        </button>
                        <button id="confirm-delete-btn" class="bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-6 rounded-lg transition shadow-md hover:shadow-lg">
                            Delete
                        </button>
                    </div>
                </div>
