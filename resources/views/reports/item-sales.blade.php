...
<div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 overflow-y-auto">
        <div class="container mx-auto px-4 py-8">
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-bold text-white mb-2">Sales by Item Summary</h1>
                <p class="text-gray-400">View item-wise sales performance and transaction details</p>
            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                value="{{ date('Y-m-d') }}"
                                max="{{ date('Y-m-d') }}"
                                class="w-full px-4 py-2.5 bg-gray-900 border border-gray-600 rounded-lg text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            ...
            <div class="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-xl shadow-xl overflow-hidden">
                <div class="p-6">
                    <h2 class="text-xl font-bold text-white mb-4">Item Sales Details</h2>
                    <div class="table-responsive">
                        <table class="w-full" id="salesTable">
                            <thead>
                                <tr class="border-b border-gray-700">
                                    <th class="text-center py-3 px-4 text-gray-300 font-semibold">#</th>
                                    <th class="text-left py-3 px-4 text-gray-300 font-semibold">Item Code</th>
                                    <th class="text-left py-3 px-4 text-gray-300 font-semibold">Item Name</th>
                                    <th class="text-center py-3 px-4 text-gray-300 font-semibold bg-blue-900/30">Total Qty</th>
...
<div id="itemDetailsModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20">
        <div class="bg-gray-800 rounded-xl shadow-2xl max-w-6xl w-full border border-gray-700">
            <!-- Modal Header -->
            <div class="bg-gradient-to-r from-blue-600 to-blue-800 px-6 py-4 rounded-t-xl">
                <div class="flex items-center justify-between">
                    ...
                    <button type="button" class="text-white hover:text-gray-200 transition" onclick="closeModal()">
            ...
            <div class="p-6">
                <div class="mb-4">
                    <h6 class="text-lg font-semibold text-white">Item: <span id="modalItemName" class="text-blue-400"></span></h6>
                </div>

                <div class="flex justify-end mb-4">
                    <button type="button" id="exportDetailsBtn" class="bg-green-600 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-lg transition duration-200 flex items-center">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                        </svg>
                        Export Details
                ...
                <div class="table-responsive">
                    <table class="w-full">
                        <thead>
                            <tr class="border-b border-gray-700">
                                <th class="text-center py-3 px-4 text-gray-300 font-semibold">#</th>
                                <th class="text-left py-3 px-4 text-gray-300 font-semibold">Order #</th>
                                <th class="text-left py-3 px-4 text-gray-300 font-semibold">Date & Time</th>
                                <th class="text-center py-3 px-4 text-gray-300 font-semibold">Qty</th>
                                <th class="text-right py-3 px-4 text-gray-300 font-semibold">Unit Price</th>
                                <th class="text-right py-3 px-4 text-gray-300 font-semibold">Subtotal</th>
                            </tr>
                        </thead>
                        <tbody id="detailsTableBody" class="divide-y divide-gray-700">
                            <tr>
                        ...
                        <tfoot class="border-t-2 border-gray-600">
                            <tr class="bg-gray-700/50">
                                <td colspan="3" class="text-right py-3 px-4 text-white font-bold">TOTAL:</td>
                                <td class="text-center py-3 px-4 text-white font-bold" id="modalTotalQty">0</td>
                                <td colspan="2"></td>
                            </tr>
                        </tfoot>
                    </table>
                    ...
                    <tr>
                        <td colspan="5" class="text-center py-12">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-gray-400">Error loading data. Please try again.</p>
                                <p class="text-red-400 text-sm mt-2">Status: ${xhr.status}</p>
                            </div>
                        </td>
                    </tr>
                `);
                ...
                <tr>
                    <td colspan="5" class="text-center py-12">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-gray-400">No sales data found for the selected period.</p>
                        </div>
                    </td>
                </tr>
            `;
            ...
            data.forEach((item, index) => {
                bodyHtml += `
                    <tr class="hover:bg-gray-700/30 transition">
                        <td class="text-center py-3 px-4 text-gray-300">${index + 1}</td>
                        <td class="py-3 px-4 text-gray-300">${item.item_code}</td>
                        <td class="py-3 px-4"><span class="text-white font-semibold">${item.item_name}</span></td>
                        <td class="text-center py-3 px-4"><span class="text-blue-400 font-bold">${item.total_quantity}</span></td>
                        <td class="text-center py-3 px-4">
                            <button class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg transition duration-200 flex items-center justify-center mx-auto" 
    ...
    function renderDetailsTable(transactions) {
        ...
        if (transactions.length === 0) {
            html = `
                <tr>
                    <td colspan="6" class="text-center py-8">
                        <p class="text-gray-400">No transaction details found.</p>
                    </td>
                </tr>
            `;
        } else {
            transactions.forEach((row, index) => {
                html += `
                    <tr class="hover:bg-gray-700/30 transition">
                        <td class="text-center py-3 px-4 text-gray-300">${index + 1}</td>
                        <td class="py-3 px-4"><span class="bg-gray-700 text-gray-300 px-3 py-1 rounded-full text-sm">${row.order_number}</span></td>
                        <td class="py-3 px-4 text-gray-300">${row.completed_at}</td>
                        <td class="text-center py-3 px-4 text-blue-400 font-semibold">${row.quantity}</td>
                        <td class="text-right py-3 px-4 text-gray-300">Rs. ${parseFloat(row.unit_price).toFixed(2)}</td>
                        <td class="text-right py-3 px-4 text-white font-semibold">Rs. ${parseFloat(row.subtotal).toFixed(2)}</td>
                    </tr>
                `;
            });
        }
