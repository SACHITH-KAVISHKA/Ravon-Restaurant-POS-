        ...
        <div class="flex-1 p-8">
            ...
            <div class="mb-6">
                <div class="flex justify-between items-center">
                    <div>
                        <h1
                            class="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#667eea] to-[#764ba2]">
                            Category Management</h1>
                        <p class="text-gray-600 mt-1">Manage food and beverage categories</p>
                    </div>
                    <button onclick="openAddCategoryModal()"
                        class="bg-gradient-to-r from-[#667eea] to-[#764ba2] hover:shadow-lg hover:shadow-purple-500/50 text-white font-semibold px-6 py-2.5 rounded-lg transition duration-200 flex items-center gap-2">
                                ...
                                <tr class="hover:bg-purple-50 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            <button
                                                onclick='editCategory({{ $category->id }}, "{{ addslashes($category->name) }}", "{{ addslashes($category->description ?? '') }}", {{ $category->display_order ?? 0 }})'
                                                class="inline-flex items-center px-3 py-2 bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white rounded-lg hover:shadow-md transition"
                                                title="Edit">
                                            ...
                                            <form action="{{ route('menu.categories.destroy', $category) }}" method="POST"
                                                onsubmit="return confirm('Are you sure? This will affect all items in this category!')">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
    ...
    <div id="categoryModal" class="hidden fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center">
        <div class="bg-white rounded-xl p-6 max-w-md w-full mx-4 shadow-2xl border border-gray-200">
            <div class="flex justify-between items-center mb-4">
                <h2 class="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-[#667eea] to-[#764ba2]"
                    id="modalTitle">Add Category</h2>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="text" name="name" id="categoryName" required
                            class="w-full px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 placeholder-gray-400">
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Description</label>
                        <textarea name="description" id="categoryDescription" rows="3"
                            class="w-full px-4 py-3 bg-white text-gray-900 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 placeholder-gray-400"></textarea>
                    </div>
