            ...
            <script>
                ...
                qz.security.setCertificatePromise(function (resolve, reject) {
                    resolve(`-----BEGIN CERTIFICATE-----
                                                        MIIDozCCAougAwIBAgIU    WJpvpJOkleU6lWsqrMKfsq9u6OowDQYJKoZIhvcNAQEL
                                                        BQAwYTELMAkGA1UEBhMC    TEsxEDAOBgNVBAgMB1dlc3Rlcm4xEDAOBgNVBAcMB0Nv
                                                        bG9tYm8xFTATBgNVBAoM    DFJhdm9uIEJha2VyczEXMBUGA1UEAwwOMTI3LjAuMC4x
                                                        OjgwMDAwHhcNMjUxMTE3    MTgwNzI0WhcNMzUxMTE1MTgwNzI0WjBhMQswCQYDVQQG
                                                        EwJMSzEQMA4GA1UECAwH    V2VzdGVybjEQMA4GA1UEBwwHQ29sb21ibzEVMBMGA1UE
                                                        CgwMUmF2b24gQmFrZXJz    MRcwFQYDVQQDDA4xMjcuMC4wLjE6ODAwMDCCASIwDQYJ
                                                        KoZIhvcNAQEBBQADggEP    ADCCAQoCggEBANF0JduabBoiZ1M7R28FmCmvUEDYy+2z
                                                        uz+zQZiBGT3pm3gD2HgZ    fvhooGywwX2lmEn5Q5wvq3dodcqpd+Nr7xDE6U2QEcGS
                                                        UEi0aDbTCBY2VIRP5HNP    33hDqNOq06akEtJRxGQ43hOLxoSWZjYxe7hIstVfp2fU
                                                        4j+uycPv9E8Cxo6eIM6N    CFfRN1mIbkIIjgVfAmOaJb1y+TbD8z5NxXAfPf31GvXi
                                                        7AJ3gnr6khs6XyW5umce    sBeOijBL+lUyTRU26GQWiduoaeoTToN9UkX3ZEvfPlR7
                                                        YLYqfRHnT4RJxRs+BcTD    Msy0JHI5MGD/Ur/u8uXNgK2mqrfPLado9y0CAwEAAaNT
                                                        ...
                                                        BQADggEBADlwDYAu7LGz    j+pGROVavOeVczrb8RibbIbXrIViV31iKC1uwXRmtTY1
                                                        amAX+oEfMry3TIy//BHs    JzGkAd6ozfosez33G4bbN8/y1Q9ZvcuaaHPT4DIBYrdR
                                                        GX/B6TtAm63VxXyjfwrV    4OUbbqwdgMtKuviRprB9A+oCE1QPa74p33hgy8UHYOCK
                                                        g9lFgnRkyrLOb4fh2Smt    jHhRV4aZf5CM+UbqBQAMiiuhHLAbqbmhBP3BYzVVZ066
                                                        9moVkpDvvNADqW3FH6ep    eBDL8RyQXj2yikCyD3xXJIAih815xLJMh/pOmuqEjHdd
                                                        NESCtDma6uLcth74mGaB    wU3G3KsOCP4=
                                                    -----END CERTIFICATE    -----`);
                });
                ...
                function renderBill() {
                    ...
                    if (billItems.length === 0) {
                        billItemsDiv.innerHTML = `
                                                            <div class="text-cen    ter text-gray-500 py-8">
                                                                <svg class="w-16 h-1    6 mx-auto mb-2 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    billItemsDiv.innerHTML = billItems.map((item, index) => {
                        ...
                        return `
                                                        <div class="bg-gray-    700 rounded-lg p-3 border border-gray-600 ${isOriginalItem && item.quantity <= originalQty ? 'border-l-4 border-l-orange-500' : ''}">
                                                            <div class="grid gri    d-cols-3 gap-2 text-sm">
                                                                <div class="col-span    -2">
                                                                    <div class="font-sem    ibold text-white">${index + 1}. ${item.name}</div>
                                                                ...
                                                                <div class="text-cen    ter">
                                                                    <div class="flex ite    ms-center justify-center space-x-2">
                                                                        <button ${decrementO    nclick
                    } class= "${decrementBtnClass}" > -</button >
                                                                        <span class="text-wh    ite font-semibold">${item.quantity}</span>
                                                                        <button onclick="inc    rementQuantity(${index})" class="w-6 h-6 bg-green-600 text-white rounded hover:bg-green-700">+</button>
                                                                    </div >    
                                                                </div >    
                                                            </div >
                        <div class="text-rig    ht mt-2 text-white font-semibold">
                    ...
                    // Cancel a voided order (when user starts a new order without adding items)
                        ...
                        function showPortionSelection(itemId, itemName, basePrice, portions) {
                            ...
                            optionsDiv.innerHTML = portions.map(p => {
                                ...
                                return `
                        < button class= "p-3 b    g-blue-700 text-white rounded-lg hover:bg-blue-600 transition font-semibold"
                                                                onclick = "addPortionT    oBill(${itemId}, '${itemName}', ${portionPrice}, '${p.name}', ${p.id})" >
                        ${ p.name }    
                                                        </button >
                        `;    
                            }).join('');
...
>>>>>>> a8f3784c9b95424f1b2d613e92ea5d6d6936fb9d
                            document.getElementById('menuSelectionContainer').classList.remove('hidden');
                            document.getElementById('menuSelectionContainer').classList.add('flex');
                            const msg = document.getElementById('initialStateMessage');
                            if (msg) msg.classList.add('hidden');
                            ...
                            try {
                                ...
                                if (result.success) {
                                    ...
                                    tableGrid.innerHTML = result.tables.map(table => {
                                        ...
                                        return `
                    < button    
                                                                        ${ clickable? `oncli    ck="selectTable('${table.table_number}', ${table.id})"` : 'disabled'}
                class="p-4 ${bgColor    } text-white rounded-lg transition font-semibold" >
                    ${ table.table_number }
                                                                        ${ !table.is_availabl    e ? '<br><span class="text-xs">(Reserved)</span>' : '' }
                                                                    </button >
                    `;    
                                    }).join('');
                    ...
                    < div class="bg-gray-    700 rounded-lg p-4 hover:bg-gray-600 cursor-pointer transition"
                onclick = "loadOrder($    {order.id})" >
                    <div class="flex jus    tify-between items-center">
                        <div>
                            <div class="text-whi    te font-semibold">${order.order_number}</div>
                            <div class="text-sm     text-gray-400">
                                ${typeDisplay} | ${o    rder.items_count} items
                            </div>
                        </div>
                        <div class="text-rig    ht">
                            <div class="text-whi    te font-bold">Rs. ${parseFloat(order.total_amount).toFixed(2)}</div>
                            <div class="text-xs     text-gray-400">${order.created_at}</div>
                        </div>
                    </div>    
                                                                    </div >
                            ...
                            try {
                                ...
                                if (result.success) {
                                    ...
                                    tableGrid.innerHTML = result.tables.map(table => {
                                        ...
                                        return `
                    < button    
                                                                        ${ clickable ? `oncli    ck="confirmTableTransfer('${table.table_number}', ${table.id})"` : 'disabled' }
                class="p-4 ${bgColor    } text-white rounded-lg transition font-semibold" >
                    ${ table.table_number }
                                                                        ${ label }    
                                                                    </button >
                            ...
                            try {
                                ...
                                if (result.success) {
                                    ...
                                    if (availableOrders.length === 0) {
                                        mergeGrid.innerHTML = `
                    < div class="col-span    -3 text-center text-gray-500 py-8" >
                                                                        <svg class="w-16 h-1    6 mx-auto mb-4 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                            <path stroke-linecap    ="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                                                        </svg>    
                                                                        <p class="text-lg fo    nt-semibold">No other orders available to merge</p>
                                                                        <p class="text-sm te    xt-gray-400 mt-1">All open orders are currently unavailable for merging</p>
                                                                    </div >
                    `;    
                                    } else {
                                        mergeGrid.innerHTML = availableOrders.map(order => {
                                            ...
                                            return `
                    < button
                onclick = "selectOrder    ToMerge(${order.id})"
                class="p-4 bg-gray-7    00 hover:bg-teal-600 text-white rounded-lg transition border-2 border-gray-600 hover:border-teal-500 text-left" >
                                                                            <div class="flex jus    tify-between items-start mb-2">
                                                                                <div class="font-bol    d text-lg">${order.order_number}</div>
                                                                                <span class="text-xs     px-2 py-1 rounded ${typeBadgeColor}">${typeDisplay}</span>
                                                                            </div>    
                                                                            <div class="text-sm     text-gray-300 mb-2">
                                                                                <span class="inline-    flex items-center">
                                                                            ...
                                                                            <div class="flex jus    tify-between items-center mt-2 pt-2 border-t border-gray-600">
                                                                                <span class="text-xs     text-gray-400">${order.created_at}</span>
                                                                                <span class="font-bo    ld text-teal-400">Rs. ${parseFloat(order.total_amount).toFixed(2)}</span>
                                                                            </div>    
                                                                        </button >
                    `;    
                                        }).join('');
                        ...
                        function renderVoidItemsTable() {
                            ...
                            if (voidItemsList.length === 0) {
                                tbody.innerHTML = `
                    < tr id = "noVoidItemsR    ow" >
                        <td colspan="4" clas s="px-4 py-6 text-center text-gray-400">
                            No items added to vo    id. Select items above.
                        </td>    
                                                            </tr >
                    `;    
                                return;
                    ...
                    < tr class="border-t     border-gray-600" >
                                                            ...
                                                            <td class="px-4 py-3     text-center text-orange-400 font-bold">-${item.void_quantity}</td>
                                                            <td class="px-4 py-3     text-center">
                                                                <button onclick="rem    oveVoidItem(${index})" class="text-red-400 hover:text-red-300 transition">
                                                                    <svg class="w-5 h-5"     fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                        <path stroke-linecap    ="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                                    </svg>    
                                                                </button>    
                    ...
                    < !DOCTYPE html >
                        <html>
                            <head>
                                <meta charset="UTF-8    ">
                                    <title>Receipt - ${o    rder.order_number || 'Order #' + order.id}</title>
                                    <style>
                                        * {
                                            margin: 0;
                                        padding: 0;
                                        ...
                                        body {
                                            font - family: 'Courie    r New', Courier, monospace;
>>>>>>> a8f3784c9b95424f1b2d613e92ea5d6d6936fb9d
                                        font-size: 11px;
                                        line-height: 1.3;
                                        width: 80mm;
...
=======
                                        ...
                                        .divider {
                                            border - top: 1px dash    ed #000;
                                        margin: 5px 0;    
                                            }

                                        .divider-thick {
                                            border - top: 2px soli    d #000;
                                        margin: 5px 0;    
                                            }
                            ...
                            <body>
                                <div class="header">
                                    ...
                                    <div class="subtitle    ">Ravon Restaurant (Pvt) Ltd</div>
                                    <div class="address"    >
                                        NO 282/A/2, KCTHALAW    ALA,<br>
                                            KADUWELA.<br>
                                                TEL.016-2006007<br>
                                                    Email-ravonrestauran    t@gmail.com
                                                </div>
                                            </div>
                                            ...
                                            <div class="section"    >
                                                <div class="info-row    ">
                                                    <span class="label">    Invoice #</span>
                                                    <span>${order.order_    number || order.id}</span>
                                                </div>
                                                <div class="info-row    ">
                                                    <span class="label">    Date</span>
                                                    <span>:${dateStr} Ti    me ${timeStr}</span>
                                                </div>
                                                <div class="info-row    ">
                                                    <span class="label">    Terminal:</span>
                                                    <span>01</span>
                                            ...
                                            <div class="items-he    ader">
                                                <span>In Item Price<    /span>
                                                    <span>Qty Amount</sp    an>
                                            </div>
                                            ...
                                            <div class="totals">
                                                <div class="total-ro    w">
                                                    <span>Sub Total</spa    n>
                                                    <span>${parseFloat(o    rder.subtotal).toFixed(2)}</span>
                                                </div>
                                            </div>

                                            <div class="divider-    thick"></div>

                                            <div class="total-ro    w grand">
                                                <span>Total</span>
                                                <span>${parseFloat(o    rder.total_amount).toFixed(2)}</span>
                                            </div>

                                            <div class="payment-    info">
                                                <div class="total-ro    w">
                                                    <span>Payment Method    </span>
                                                    <span>${paymentMetho    d}</span>
                                                </div>
                                                ${cashAmount > 0 ? `    
                                            <div class="total-ro    w">
                                                <span>Cash</span>    
                                                <span>${parseFloat(c    ashAmount).toFixed(2)}</span>
                                            </div>    
                                            ` : ''}
                                                ${cardAmount > 0 ? `    
                                            <div class="total-ro    w">
                                                <span>Card</span>    
                                                <span>${parseFloat(c    ardAmount).toFixed(2)}</span>
                                            </div>    
                                            ` : ''}
                                                ${creditAmount > 0 ? `
                                            <div class="total-ro    w">
                                                <span>Credit</span>    
                                                <span>${parseFloat(c    reditAmount).toFixed(2)}</span>
                                            </div>    
                                            ` : ''}
                                                ${parseFloat(changeA    mount) > 0 ? `
                                            <div class="total-ro    w">
                                                <span>Change</span>    
                                                <span>${parseFloat(c    hangeAmount).toFixed(2)}</span>
                                            </div>    
                                            ` : ''}
                                            </div>

                                            <div class="footer">
                                                <div style="font-wei    ght: bold; margin-bottom: 5px;">THANK YOU, COME AGAIN.</div>
                                                <div class="footer-n    ote">
                                                    Software By SKM Labs
                                                </div>
                                            </div>
                                        </body>
                                            ...
                                            return `
                                                                    <div class="bg-gray-    700 rounded-lg p-4 mb-2 flex justify-between items-center hover:bg-gray-650 transition">
                                                                        <div>    
                                                                            <div class="text-whi    te font-semibold">${order.order_number}</div>
                                                                            <div class="text-sm     text-gray-400">
                                                                                ${typeDisplay} | ${o    rder.items_count
                                        } items | ${ order.payment_method.toUpperCase() }
                                                                            </div>
                            </div>
                            <div class="flex ite    ms-center gap-4">
                                <div class="text-rig    ht">
                                    <div class="text-whi    te font-bold">Rs. ${parseFloat(order.total_amount).toFixed(2)}</div>
                                    <div class="text-xs     text-gray-400">${order.completed_at}</div>
                                </div>
                                <button onclick="pri    ntReceipt(${order.id})"
                                    class="bg-rose-500 h    over:bg-rose-600 text-white p-2 rounded-lg transition shadow-sm"
                                    title="Print Receipt    ">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                                    </svg>
